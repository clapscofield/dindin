...
import InvestimentosManager from "features/inserirInvestimento/InvestimentoManager";
...
const VisualizarInvestimentos = (props) => {
  const { usuario } = props;
  ...
  useEffect(() => {
    const removerTransacao = async () => {
      if (idTransacao) {
        const resultado = await InvestimentosManager.removerInvestimento(
          idTransacao
        );

        if (resultado) {
          console.log(resultado);
          setRedirecionar(<Redirect to={"/visualizar-investimentos"} />);
        }
      }
    };
  ...
  useEffect(() => {
    const obterTransacoes = async () => {
      ...
      const resultado = await InvestimentosManager.listarInvestimentos(user);
...
export default connect(mapStateToProps)(VisualizarInvestimentos);
